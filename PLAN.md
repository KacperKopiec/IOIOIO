...
| Role | **3 role**: `promocja` (Dział promocji), `koordynator` (Koordynator wydarzenia), `opiekun` (Opiekun partnerów). Ekrany Figma „Kadra zarządzająca” (105:3609) i „Osoba merytoryczna” (105:4792) są poza zakresem. |
...
- `AuthContext`: trzyma `role: 'koordynator' | 'opiekun' | 'promocja'`, hard-coded `'koordynator'`. Brak przełącznika dev mode (do dodania w Fazie 3).
- Routing: `/dashboard`, `/firms`, `/events`. Brak `/pipeline`, `/companies/:id`, `/events/:id`, `/reports`.

### Figma (`1OBvTrBmTXd0BL5FpPT1ZX`, canvas `105:3608`)
W zakresie produktu jest 9 ekranów (2 z Figmy – Kadra zarządzająca `105:3609` i Osoba merytoryczna `105:4792` – pominięte, bo te role nie istnieją).

| # | nodeId | Nazwa | Stan UI |
|---|---|---|---|
| 1 | `105:3978` | Baza Firm – All | **w UI**, mock |
| 2 | `105:4451` | Lejek (pipeline kanban) | **brak** |
| 3 | `105:5107` | Widok firmy w kontekście wydarzenia | **brak** |
| 4 | `105:5431` | Opiekun partnerów – Dashboard | stub |
| 5 | `105:5721` | Raporty | **brak** (widok dla koordynatora) |
| 6 | `105:6030` | Wydarzenia – lista | **w UI**, mock |
| 7 | `105:6344` | Szczegóły firmy (taby: Dane Kontaktowe / Historia Współpracy / Notatki) | **brak** |
| 8 | `105:6590` | Dashboard – Dział promocji | stub |
| 9 | `105:6851` | Koordynator wydarzenia – widok wydarzenia (event detail / dashboard koordynatora) | **brak** |
...
4. Seedy (`db/seeds.py` wywoływany przez `python -m app.db.seeds`):
   ...
   - 3 użytkowników demo (po jednym na rolę).
...
5. Dodać dev-mode role switcher do `AuthContext` / `TopBar` (dropdown z 3 rolami, widoczny tylko gdy `import.meta.env.DEV`).
...
5. **Dashboardy ról** (3 role):
   - `DashboardCoordinator` – Figma `105:6851` zaadaptowany do widoku „wybierz wydarzenie”.
...
- [ ] Wszystkie 9 ekranów w zakresie produktu mają routing + UI + zaciągają realne dane (jeden ekran może mieć stub jeśli wymaga decyzji produktowej – odznaczyć w PR).
- [ ] Dashboard koordynatora pokazuje **realne KPI** (liczba partnerów / suma sponsoringu / konwersja) zamiast stałych.
